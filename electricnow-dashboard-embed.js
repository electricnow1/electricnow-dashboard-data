(() => {
  const mountId = 'electricnow-dashboard-hosted-mount';
  const existing = document.getElementById('electricnow-dashboard-embed');
  if (existing) existing.remove();
  let mount = document.getElementById(mountId);
  if (!mount) {
    mount = document.createElement('div');
    mount.id = mountId;
    document.currentScript?.insertAdjacentElement('beforebegin', mount);
  }
  if (!document.querySelector('link[data-electricnow-fonts]')) {
    const pre1 = document.createElement('link'); pre1.rel = 'preconnect'; pre1.href = 'https://fonts.googleapis.com'; pre1.dataset.electricnowFonts = 'true'; document.head.appendChild(pre1);
    const pre2 = document.createElement('link'); pre2.rel = 'preconnect'; pre2.href = 'https://fonts.gstatic.com'; pre2.crossOrigin = ''; pre2.dataset.electricnowFonts = 'true'; document.head.appendChild(pre2);
    const font = document.createElement('link'); font.rel = 'stylesheet'; font.href = 'https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@500;600;700;800&family=Inter:wght@400;500;600;700&display=swap'; font.dataset.electricnowFonts = 'true'; document.head.appendChild(font);
  }
  let style = document.getElementById('electricnow-dashboard-hosted-style');
  if (!style) { style = document.createElement('style'); style.id = 'electricnow-dashboard-hosted-style'; document.head.appendChild(style); }
...
function renderPurchases() {
  ...
  const recentPaymentLine = (thisWeek?.payments || sales.recentPayments || [])
    .slice(0, 4)
    .map((payment) => `${escapeHtml(payment.date)} ${fmt.currency(payment.amount)}`)
    .join(' · ');
  $('#sales-summary').innerHTML = `
    <div class="sales-callout">
      <span>Stripe-visible sales this year</span>
      <strong>${fmt.number(yearToDate.purchases)} purchases / ${fmt.currency(yearToDate.purchaseRevenue)}</strong>
      <p>${yearToDate.range}. This is year-to-date Stripe-visible sales only; Apple App Store and Google Play purchases are not included yet.</p>
    </div>
    ...
    ${recentPaymentLine ? `<div class="sales-audit">
      <span>Most recent Stripe-visible payments</span>
      <strong>${recentPaymentLine}</strong>
      <p>These are the latest payments visible through Stripe. They remain a partial sales view until Apple App Store and Google Play sales are reconciled.</p>
    </div>` : ''}
